...
selected_hs_code = st.sidebar.selectbox("Select HS Code", df["hs_code"].unique())
selected_origin = st.sidebar.selectbox("Select Current Origin Country", df["origin_country"].unique())
base_cost = st.sidebar.number_input("Base cost per TEU",step=100)
...
if subset.empty:
    ...
    estimated_cost = base_cost * (1 + hypo_tariff)
    ...
    col2.metric("Current Origin:", f"{selected_origin.capitalize()}")
